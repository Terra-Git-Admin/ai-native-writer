...
export default function PipelinePlayground({
  ...
  const effectivePipelineBranch: PipelineBranch = isAdmin ? pipelineBranch : "monetization";
  ...
  const runConnectStory = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/ai/edit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ messages, mode: effectivePipelineBranch === "regular" ? "pipe_continuation_logic" : "pipe_causality", modelId, thinking }),
      });
  ...
  }, [worldStateTab, beatSeqTab, state.instructions, plotsTab, modelId, thinking, effectivePipelineBranch, scheduleFlush]);
  ...
  const branchCopy = effectivePipelineBranch === "regular"
    ? {

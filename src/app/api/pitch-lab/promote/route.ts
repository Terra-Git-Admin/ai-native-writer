...
export async function POST(req: Request) {
  ...
  if (!ideaId || !title || !ideaText) return NextResponse.json({ error: "Idea title and plot are required." }, { status: 400 });
  ...
  try {
    const documentId = db.transaction((tx) => {
      ...
      if (idea.status !== "shortlisted") throw new Error("Shortlist this idea before promoting it.");
      ...
      plotTab.content = paragraphDoc(title, ideaText);
      ...
      tx.update(pitchIdeas).set({
        title, ideaText, status: "promoted", promotedDocumentId: id, updatedAt: now,
